...
class AppConfig:
    ...
    def normalized(self) -> "AppConfig":
        ...
        self.command_player_1, self.command_player_2, self.command_player_3 = [command or defaults[index] for index, command in enumerate(commands)]
        for player in (1, 2, 3):
            ...
            size_key = f"character_size_{player}"; intensity_key = f"character_intensity_{player}"
            x_key = f"character_x_{player}"; y_key = f"character_y_{player}"
            ...
            idle_key = f"character_idle_{player}"; idle = str(getattr(self, idle_key)).strip().lower()
            setattr(self, idle_key, idle if idle in IDLE_ANIMATIONS else "float")
            speaking_key = f"character_speaking_{player}"; speaking = str(getattr(self, speaking_key)).strip().lower()
            setattr(self, speaking_key, speaking if speaking in SPEAKING_ANIMATIONS else "auto")
        return self

    def to_dict(self) -> dict[str, Any]:
        data = asdict(self); extra = data.pop("extra", {}); data.update(extra); return data
    ...
    def from_dict(cls, data: dict[str, Any]) -> "AppConfig":
        ...
        cfg = cls(**values); cfg.extra = extra; return cfg.normalized()


def character_file_path(config: AppConfig, player: int) -> Path | None:
    if player not in (1, 2, 3): return None
...
def install_character_image(player: int, source: str | Path) -> str:
    if player not in (1, 2, 3): raise ValueError("Jogador inválido.")
    source_path = Path(source).expanduser().resolve()
    if not source_path.is_file(): raise ValueError("A imagem escolhida não existe.")
    suffix = source_path.suffix.lower()
    if suffix not in ALLOWED_CHARACTER_EXTENSIONS: raise ValueError("Use PNG, JPG, JPEG, WebP ou GIF.")
    if source_path.stat().st_size > CHARACTER_MAX_BYTES: raise ValueError("A imagem deve ter no máximo 25 MB.")
    CHARACTER_DIR.mkdir(parents=True, exist_ok=True)
    for old in CHARACTER_DIR.glob(f"jogador{player}.*"):
        try: old.unlink()
        except OSError: pass
    target = CHARACTER_DIR / f"jogador{player}{suffix}"; shutil.copy2(source_path, target); return target.name


def remove_character_image(player: int) -> None:
    if player not in (1, 2, 3) or not CHARACTER_DIR.exists(): return
    for old in CHARACTER_DIR.glob(f"jogador{player}.*"):
        try: old.unlink()
        except OSError: pass
...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    if not target.exists(): return AppConfig()
    try:
        data = json.loads(target.read_text(encoding="utf-8"))
        if not isinstance(data, dict): raise ValueError("configuração inválida")
        return AppConfig.from_dict(data)
    except (OSError, ValueError, TypeError, json.JSONDecodeError): return AppConfig()
...
def save_config(config: AppConfig, path: Path | None = None) -> Path:
    ...
    temp.write_text(payload + "\n", encoding="utf-8"); temp.replace(target); return target
