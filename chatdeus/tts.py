...
class TTSManager:
    ...
    def azure_ready(self) -> bool:
        return bool(self.config.azure_enabled and self.config.azure_key.strip() and self.config.azure_region.strip())

    def _parse_prefix(self, text: str, style: str) -> tuple[str, str]:
        stripped = text.strip()
        lower = stripped.lower()
        for prefix, prefix_style in PREFIX_STYLES.items():
            if lower.startswith(prefix):
                return stripped[len(prefix):].lstrip(), prefix_style
        return stripped, style

    def synthesize(self, text: str, voice: str, style: str = "default") -> Path | None:
        text, style = self._parse_prefix(text, style)
        if not text:
            return None
        if style == "random":
        ...
        if self.azure_ready:
            try:
                ...
                log.exception("Falha no Azure TTS; tentando fallback.")

        if self.config.fallback_gtts:
            try:
                return self._gtts(text)
            except Exception:
                log.exception("Falha no fallback gTTS.")
        return None

    def _temp_path(self, suffix: str) -> Path:
        folder = Path(tempfile.gettempdir()) / "ChatDeusApp"
    ...
    def _azure(self, text: str, voice: str, style: str) -> Path | None:
        ...
        if style == "default":
            ...
            inner = f"<mstts:express-as style='{html.escape(style)}'>{safe_text}</mstts:express-as>"

        ssml = (
            "<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' "
            "xmlns:mstts='http://www.w3.org/2001/mstts' xml:lang='pt-BR'>"
            f"<voice name='{html.escape(voice)}'>{inner}</voice></speak>"
        )
        result = synthesizer.speak_ssml_async(ssml).get()
        if result.reason != speechsdk.ResultReason.SynthesizingAudioCompleted and style != "default":
            plain_ssml = (
                "<speak version='1.0' xmlns='http://www.w3.org/2001/10/synthesis' xml:lang='pt-BR'>"
                f"<voice name='{html.escape(voice)}'>{safe_text}</voice></speak>"
            )
            result = synthesizer.speak_ssml_async(plain_ssml).get()

        if result.reason != speechsdk.ResultReason.SynthesizingAudioCompleted:
            details = getattr(result, "cancellation_details", None)
            log.warning("Azure TTS não concluiu a síntese: %s", details or result.reason)
            return None

        output = self._temp_path(".wav")
        stream = speechsdk.AudioDataStream(result)
        stream.save_to_wav_file(str(output))
        return output

    def _gtts(self, text: str) -> Path:
        from gtts import gTTS
        output = self._temp_path(".mp3")
        gTTS(text=text, lang="pt-br", slow=False).save(str(output))
        return output
